...
public class HistoryDAO {
    ...
    public boolean addHistoryEntry(HistoryEntry entry) {
        ...
        try (Connection conn = dbManager.getConnection();
            ...
            pstmt.setString(1, entry.getUsername()); //setzt wert ?
    ...
    public List<HistoryEntry> getHistoryForUser(String username) {
        List<HistoryEntry> entries = new ArrayList<>(); //leere liste
        String sql = "SELECT * FROM history WHERE username = ? ORDER BY timestamp DESC";
        try (Connection conn = dbManager.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, username);
            ResultSet rs = pstmt.executeQuery(); //sql query ausgeführt
            while (rs.next()) {
    ...
    public int getTotalPushupsForUser(String username) {
        String sql = "SELECT SUM(count) as total FROM history WHERE username = ? AND exercise_name = 'PushUps'";
        try (Connection conn = dbManager.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, username);
